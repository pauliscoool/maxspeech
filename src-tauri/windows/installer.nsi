...
; MaxSpeech: do NOT define MUI_FINISHPAGE_RUN.
; Tauri's RunAsUser blocks until the process exits; MaxSpeech is a tray app, so the
; finish-page "Run" checkbox hangs the installer forever. Launch is handled by
; NSIS_HOOK_POSTINSTALL (async ShellExecute) instead.
!define MUI_PAGE_CUSTOMFUNCTION_PRE SkipIfPassive
...
Function .onInstSuccess
 ; `/R` for silent/passive: async launch only (never RunAsUser — tray apps hang it).
 ; In-app updater uses /S /UPDATE without /R; POSTINSTALL already starts the app.
 ${If} $PassiveMode = 1
 ${OrIf} ${Silent}
 ${GetOptions} $CMDLINE "/R" $R0
 ${IfNot} ${Errors}
 ${GetOptions} $CMDLINE "/ARGS" $R0
 System::Call 'shell32::ShellExecuteW(i 0, w "open", w "$INSTDIR\${MAINBINARYNAME}.exe", w "$R0", w "$INSTDIR", i 1)'
 ${EndIf}
 ${EndIf}
FunctionEnd
