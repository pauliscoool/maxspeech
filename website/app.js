(() => {
  ...
  function paintOverlayWaves(now) {
    const t = now / 1000;
    document.querySelectorAll(".ms-overlay-wave").forEach((root) => {
      if (!root.offsetParent && root.getAttribute("data-active") !== "true") return;
  ...
  function tickFlow(now) {
    ...
    if (!reduceMotion) {
      if (flowVisible && stage && stage.offsetParent) {
  ...
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden && !flowRaf) flowRaf = requestAnimationFrame(tickFlow);
  });
  flowRaf = requestAnimationFrame(tickFlow);
  ...
  (function setupDownloadCta() {
    ...
    const WIN =
      "https://github.com/pauliscoool/maxspeech/releases/latest/download/MaxSpeech_x64-setup.exe";
    const ICONS = {
      windows:
      ...
      android:
        '<svg class="os-svg os-android" viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M17.6 9.48 19.44 6.3a.64.64 0 0 0-.26-.85.64.64 0 0 0-.83.22l-1.88 3.24a11.43 11.43 0 0 0-8.94 0L5.65 5.67a.64.64 0 0 0-.87-.2.64.64 0 0 0-.22.83L6.4 9.48A10.81 10.81 0 0 0 1 18h22a10.81 10.81 0 0 0-5.4-8.52ZM7 15.25a1.25 1.25 0 1 1 0-2.5 1.25 1.25 0 0 1 0 2.5Zm10 0a1.25 1.25 0 1 1 0-2.5 1.25 1.25 0 0 1 0 2.5Z"/></svg>',
      ios:
        '<svg class="os-svg os-mac" viewBox="0 0 24 24" width="18" height="18"><path fill="currentColor" d="M16.4 13.1c0-2.3 1.9-3.4 2-3.5-1.1-1.6-2.8-1.8-3.4-1.8-1.4-.2-2.8.9-3.5.9-.7 0-1.9-.8-3.1-.8-1.6 0-3.1 1-3.9 2.4-1.7 2.9-.4 7.2 1.2 9.6.8 1.1 1.7 2.4 3 2.4 1.2 0 1.6-.8 3-.8s1.7.8 3 .8c1.2 0 2-1.1 2.8-2.2.9-1.3 1.2-2.5 1.3-2.6-.03-.01-2.4-1-2.4-4.4zM14.3 5.7c.7-.8 1.1-1.9 1-3-.9.1-2.1.6-2.8 1.4-.6.7-1.2 1.8-1 2.9 1 .1 2.1-.5 2.8-1.3z"/></svg>',
    };
    ...
    else if (/iphone|ipod|ipad/.test(ua) || (p.includes("mac") && touchy)) os = "ios";
    ...
    document.documentElement.classList.toggle("is-mobile-os", mobileOs);
    ...
    if (os === "mac") {
      ...
      primary.removeAttribute("download");
      primary.classList.add("is-soon");
      primary.setAttribute("aria-disabled", "true");
      primary.addEventListener("click", (e) => e.preventDefault());
      icon.innerHTML = ICONS.ios;
      label.textContent = "Download for iOS — coming soon";
      if (auto) auto.textContent = "iPhone detected · iOS app isn’t out yet";
      if (meta) {
        meta.textContent = "The iPhone app is on the way. MaxSpeech is on Android today.";
      }
    } else if (os === "android") {
      primary.href = "/android";
      primary.removeAttribute("download");
      primary.classList.remove("is-soon");
      icon.innerHTML = ICONS.android;
      label.textContent = "Get MaxSpeech for Android";
      if (auto) auto.textContent = "Auto-select · Android detected";
      if (meta) {
        meta.innerHTML =
